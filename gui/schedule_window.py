"""
Schedule management window (popup).

Standalone window for managing schedules.
"""
...
class ScheduleWindow(tk.Toplevel):
    """Standalone schedule management window."""

    def __init__(self, parent, executor_ref):
        """
        Initialize schedule window.
    ...
    def _build_ui(self):
        ...
        ttk.Label(controls_frame, text="Schedules:", font=('', 10, 'bold')).pack(side=tk.LEFT, padx=(0, 10))
        ...
        # Close button
        ttk.Button(status_frame, text="Close", command=self._on_close).pack(side=tk.RIGHT)
    ...
    def _restore_window_geometry(self):
        """Restore window size and position from parent GUI settings."""
        if hasattr(self.parent_gui, 'gui_settings'):
            settings = self.parent_gui.gui_settings
            geometry = settings.get('schedule_window_geometry', '1200x600+100+100')
            self.geometry(geometry)
        else:
            self.geometry("1200x600")

    def _save_window_geometry(self):
        """Save window size and position to parent GUI settings."""
        if hasattr(self.parent_gui, 'gui_settings'):
            geometry = self.geometry()
            self.parent_gui.gui_settings['schedule_window_geometry'] = geometry

    def _on_close(self):
        """Handle window close - save geometry and destroy."""
